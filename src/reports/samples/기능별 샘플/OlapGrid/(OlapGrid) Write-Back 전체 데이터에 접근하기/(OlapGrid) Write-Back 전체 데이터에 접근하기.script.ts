...
const initControlVariables = function (): void {
  ...
  tbxLockCellFormula.OnTextChange = function (_s: unknown, _e: unknown): void {
...
const initWriteBack = function (): void {
  ...
  olapGrid.Options.WriteBackFetchAllRows = true; //true : 수정 셀여부와 상관없이 전체 데이터 row에 대해 접근을 허용합니다.(기본값:false)
 ...
 * * arguments :
 *****************************************/
const OnDocumentLoadComplete = function (_sender: unknown, _args: unknown): void {
...
const OnButtonClick = function (_sender: unknown, args: ButtonClickArgs): void {
  if (args.Id == "btnUpate") {
    //전체 데이터 대상이므로 수정 여부 확인하지 않음
...
const saveOlap = function (): void {
  Matrix.RunScript("OlapGrid", "Service1", function (p: RunScriptResult): void {
    if (p.Success == false) {
 ...
 *****************************************/
const OnComboBoxValueChanged = function (_sender: unknown, _args: unknown): void {
 ...
 *****************************************/
interface CheckValueChangeArgs {
  Id: string;
  IsChecked: boolean;
}

const OnCheckValueChange = function (_sender: unknown, args: CheckValueChangeArgs): void {
 ...
 *****************************************/
const OnOlapDataCellStartEdit = function (_sender: unknown, _args: unknown): void {};
 ...
 *****************************************/
interface OlapDataCellEndEditArgs {
  Id: string;
  Cell: {
    Field: {
      Format: string;
...
const OnOlapDataCellEndEdit = function (_sender: unknown, args: OlapDataCellEndEditArgs): void {

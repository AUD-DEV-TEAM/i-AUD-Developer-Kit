...
**************************************/
 var OnDocumentLoadComplete  = function(sender, args){
	...
	GRD_DETAIL.GetField('CODE').KeyType = 3;
 	...
 	/* 초기 세팅 */
 	(Matrix.getObject('LBL_DTL_CNT') as Label).Text = '0';
 ...
 var OnButtonClick  = function(sender, args){
 	switch(args.Id){
		...
		case 'BTN_GRP_SAV': // 추가 (Form: '그룹 추가')
			let grpFields = [VS_INP_GROUP_CODE.Text,VS_INP_GROUP_NAME.Text];
		...
		case 'BTN_CD_SAV': // 추가 (Form: '코드 추가')
			let cdFields = [VS_INP_CODE.Text,VS_CODE_NAME.Text];
			if(isInvalidInput(cdFields)){
				Matrix.Information('필수 입력 항목을 확인해주세요','안내');
				return;
			}

			let scriptName = BTN_CD_SAV.Text=='저장' ? 'GRD_DETAIL_UPDATE' : 'GRD_DETAIL_INSERT';
...
**************************************/
 var OnDataBindEnd  = function(sender, args){
 	if(args.Id == 'GRD_MASTER'){
		(Matrix.getObject('LBL_MST_CNT') as Label).Text = args.RecordCount;

	}else if(args.Id == 'GRD_DETAIL'){
		(Matrix.getObject('LBL_DTL_CNT') as Label).Text = args.RecordCount;
	}
 };
...
**************************************/
 var OnGridMultiHeaderCheckBoxClicked  = function(sender, args){
  	if(args.Id == 'GRD_DETAIL'){
		let GRD = Matrix.getObject(args.Id);
		let checkValue = args.Checked ? 'Y' : 'N';

		for (let i=0; i<GRD.GetRowCount(); i++) {

...
export interface TreeGrid extends Grid {
  ...
  /**
   * 전체 레코드(필터로 인해 숨은 레코드 포함)에서 특정 위치의 레코드(행)를 반환합니다.
   *
   * @param idx 행 위치
   */
   ...
   * @event
   *
   * 트리 그리드의 트리 형태 셀을 클릭할 때 발생합니다.
   *
   * @param sender 이벤트가 발생한 트리 그리드 컨트롤
   * @param args 이벤트 인자
   *
   * Target : {@link TreeGrid}
   */

...
export interface DataGridColumn{
   ...
   * @param value 필터 조건 값. Dimension 필터의 In/NotIn은 배열, Between은 [from, to] 배열, Measure 필터는 단일 값
   * @param isAnd AND 인지 OR 인지 유무(필터 타입이 Measure일 경우에만 사용. 기본값은 true)
   */
  AddFilter(type: number, operator: string, value: string | number | string[] | number[], isAnd: boolean): void;
